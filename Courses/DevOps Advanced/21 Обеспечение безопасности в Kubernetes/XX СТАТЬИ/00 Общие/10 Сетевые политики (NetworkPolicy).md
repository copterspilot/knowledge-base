...
Pod Security Standards и Pod Security Admission — это не просто замена PSP, а **эволюция подхода к безопасности**. Они делают базовую защиту простой, встроенной и доступной для всех. Как указано в `combined.txt`, PSA — это стандарт де-факто для базовой безопасности подов. Начните с применения уровня `baseline` и постепенно переходите к `restricted`. В следующей статье мы рассмотрим, как использовать `Kyverno` для создания еще более сложных политик.

#### **Ссылки**
*   `Lapaz Raul - Learning Kubernetes Security, 2nd Edition - 2025.txt` — Глава о PSS.
*   `Burns B., Villalba E., Strebel D., Evenson L. - Kubernetes Best Practices - 2024.txt` — Глава о безопасности.
*   `Ibryam Bilgin, Huss Roland - Kubernetes Patterns, 2nd Edition - 2023.txt` — Глава о безопасности.
